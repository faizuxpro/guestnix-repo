...
const discoverPlacesSchema = z
  ...
  .superRefine((value, ctx) => {
    const hasQuery = typeof value.locationQuery === "string" && value.locationQuery.length > 0;
...
function categoryFromTags(tags: Record<string, string>): PlaceCategory {
  ...
  if (amenity === "gym" || (leisure && /^(fitness_centre|sports_centre)$/i.test(leisure))) {
    return "gym";
  }
  ...
  if (tourism && /^(attraction|viewpoint|theme_park|zoo|aquarium|artwork)$/i.test(tourism)) {
    return "attraction";
  }
  if (historic) {
    return "attraction";
  ...
  if (shop) {
    if (/^(supermarket|convenience|grocery|greengrocer|butcher|bakery)$/i.test(shop)) {
      return "grocery";
    }
    return "shopping";
  }
...
const TAG_BUCKETS: TagBucket[] = [
  // amenity is the biggest in cities, give it more headroom
  {
    id: "amenity-food",
    clauses: [
...
`;
  ...
  // Try the assigned mirror first, then fall back to the next one if it fails.
  const order = [
...
async function fetchNearbyPlaces(
  ...
  // Each bucket gets its own per-mirror timeout; smaller queries finish far
  // faster than the historical OR-combined one, so 18s is plenty per bucket
  // and the whole search resolves to whichever buckets returned in time.
